...
function Errormator() {
    ...
    this.error_report_buffer = [];
    this.slow_report_buffer = [];
    this.log_buffer = [];
    ...
    this.init = function (options) {
        ...
        if (typeof options.window_on_error == 'undefined' ||
            options.window_on_error == false) {
            TraceKit.collectWindowErrors = false;
        }
        if (typeof options.send_interval == 'undefined') {
            options.send_interval = 1000;
        }
        if (options.send_interval >= 1000){
            this.create_send_interval(options.send_interval);
        }
        this.options = options;
        this.requestInfo = null;
        this.reports_endpoint = options.server
            + '/api/reports?public_api_key=' + this.options.api_key
            + "&protocol_version=" + this.options.protocol_version;
        this.slow_reports_endpoint = options.server
            + '/api/slow_reports?public_api_key=' + this.options.api_key
            + "&protocol_version=" + this.options.protocol_version;
        this.logs_endpoint = options.server
            + '/api/logs?public_api_key=' + this.options.api_key
            + "&protocol_version=" + this.options.protocol_version;
    ...
    this.create_send_interval = function(time_iv){
        this.send_iv = setInterval(function(){
            self.send_reports();
            self.send_logs();
            self.send_slow_reports();
        }, time_iv);
    }

    this.setRequestInfo = function (info) {
        this.requestInfo = info;
    };

    this.grabError = function (exception) {
    ...
    this.log = function (level, message, namespace, uuid) {
    ...
    this.send_reports = function () {
        if (this.error_report_buffer.length < 1) {
            return true;
        }
        var data = this.error_report_buffer;
        this.submit_data(this.reports_endpoint, data);
        this.error_report_buffer = [];
        return true;
    };

    this.send_slow_reports = function () {
        if (this.slow_report_buffer.length < 1) {
            return true;
        }
        var data = this.slow_reports_buffer;
        this.submit_data(this.slow_reports_endpoint, data);
        this.slow_error_report_buffer = [];
        return true;
    };
    this.send_logs = function () {
        if (this.log_buffer.length < 1) {
            return true;
        }
        var data = this.log_buffer;
        this.submit_data(this.logs_endpoint, data);
        this.log_buffer = [];
        return true;
    };

    this.submit_data = function (endpoint, data) {
    ...
    this.collectDetails = function () {
        ...
        if (typeof detail.request_id == 'undefined' || !detail.request_id) {
            detail.request_id = this.gen_uuid4();
        }
        return detail
